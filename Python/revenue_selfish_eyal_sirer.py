...
def V_T_self_MC_ES(u, t, λ, p, q, b, c, sim_data):
    ...
    R_t_AG = np.array([u - c * T_exp[k] + b * U[k]['U_AG'][-1] for k in range(K)])
    R_t_ES = np.array([u - c * T_exp[k] + b * U[k]['U_ES'][-1] for k in range(K)])
    ...
    R_s_AG = [u - c * T[k] + b * np.insert(U[k]['U_AG'][:-1],0,0) for k in range(K)]
    R_s_ES = [u - c * T[k] + b * np.insert(U[k]['U_ES'][:-1],0,0) for k in range(K)]
